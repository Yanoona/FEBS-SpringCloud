...
import com.yan.febsauth.properties.FebsValidateCodeProperties;
...
public class ValidateCodeService {
     ...
     * 所以我们需要客户端在发送获取验证码请求的时候，携带一个key（比如按一定算法生成的随机字符串，
     * 模拟jsessionid）来和验证码一一对应。于是我们在create里一开始就从请求中获取key值，
     * 然后根据验证码配置文件生成验证码，并将验证码字符保存到了Redis中（Redis Key为 febs.captcha. + 客户端上送的key值，
    ...
    public void create(HttpServletRequest request, HttpServletResponse response) throws IOException, ValidateCodeException {
        ...
        FebsValidateCodeProperties codeProperties = febsAuthProperties.getCode();
        setHeader(response, codeProperties.getType());

        Captcha captcha = createCaptcha(codeProperties);
        redisService.set(FebsConstant.CODE_PREFIX + key, StringUtils.lowerCase(captcha.text()), codeProperties.getTime());
     ...
     * 比如PNG格式的或者GIF格式的，验证码图片的长宽高，
     * 验证码字符的类型（纯数字，纯字母或者数字字母组合），验证码字符的长度等
     * @param code
     * @return
     */
